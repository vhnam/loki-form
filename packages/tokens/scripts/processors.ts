 ...
 */
export function processColorPrimitives(primitives: Record<string, Token>, output: string[]): void {
    ...
    if (colorValue.$value) {
      ...
      for (const [scale, scaleValue] of Object.entries(colorValue)) {
        if (
          ...
          const value = processTokenValue((scaleValue as any).$value, (scaleValue as any).$type);
          output.push(`  ${varName}: ${value};`);
        }
      }
    }
  ...
  for (const [semanticName, semanticValue] of Object.entries(semantic)) {
    ...
    if (semanticValue.$value) {
      ...
      const lightValue = semanticValue.$extensions?.mode?.light
        ? processTokenValue(semanticValue.$extensions.mode.light, semanticValue.$type)
      ...
      if (semanticValue.$extensions?.mode?.dark) {
        const darkValue = processTokenValue(semanticValue.$extensions.mode.dark, semanticValue.$type);
      ...
      for (const [nestedName, nestedValue] of Object.entries(semanticValue)) {
        if (
          ...
          const lightValue = (nestedValue as any).$extensions?.mode?.light
            ? processTokenValue((nestedValue as any).$extensions.mode.light, (nestedValue as any).$type)
            : processTokenValue((nestedValue as any).$value, (nestedValue as any).$type);

          light[varName] = lightValue;

          // Generate dark mode override if available
          if ((nestedValue as any).$extensions?.mode?.dark) {
            const darkValue = processTokenValue((nestedValue as any).$extensions.mode.dark, (nestedValue as any).$type);
            dark[varName] = darkValue;
          }
        }
      }
 ...
 */
export function processFoundations(foundations: DesignTokens['foundations'], output: string[]): void {
 ...
 */
export function processShadow(shadow: DesignTokens['shadow'], output: string[]): void {
  if (!shadow) return;
