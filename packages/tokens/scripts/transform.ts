...
export function transformToTailwindTheme(tokens: DesignTokens): string {
  ...
  let output: string[] = ['@custom-variant dark (&:where(.dark, .dark *));', '', '@theme {'];
  ...
  if (tokens.color?.semantic) {
    ...
    output.push(...Object.entries(light).map(([key, value]) => `  ${key}: ${value};`));

    output = output.concat(['}', '', '@layer theme {', '', '.dark {']);

    output.push(...Object.entries(dark).map(([key, value]) => `  ${key}: ${value};`));
